"""
Train XGBoost Quantile Regression Model v8

Same architecture as v7 (5 quantile XGBRegressors) but trained on the
v6 feature matrix which includes 10 NTC (Net Transfer Capacity) features
capturing cross-border interconnector constraints.

Features: 65 D-1 12:00 CET safe features (v6 feature matrix with NTC)
Target: price_eur_mwh (continuous)

Usage:
    python scripts/train_quantile_model_v8.py
"""
...
def load_feature_matrix() -> pd.DataFrame:
    input_path = PROCESSED_DATA_DIR / "feature_matrix_v6.csv"

    if not input_path.exists():
        raise FileNotFoundError(
            f"Feature matrix not found at {input_path}. "
            "Run create_feature_matrix_v6.py first."
        )
...
def main():
    ...
    logger.info("Features: 65 D-1 12:00 CET safe features (v6 feature matrix with NTC)")
    ...
    try:
        ...
        # Log NTC features being used
        ntc_features = [c for c in feature_cols if c.startswith('ntc_')]
        logger.info(f"NTC features ({len(ntc_features)}): {ntc_features}")
        ...
        # NTC feature importance
        ntc_imp = importance[importance['feature'].str.startswith('ntc_')]
        if len(ntc_imp) > 0:
            logger.info(f"\nNTC Feature Importance:")
            for _, row in ntc_imp.iterrows():
                logger.info(f"  {row['feature']}: {row['importance']:.4f}")

        # v7 comparison (quantile metrics)
        v7_metrics_path = MODELS_DIR / "test_metrics_v7.pkl"
        if v7_metrics_path.exists():
            v7_metrics = joblib.load(v7_metrics_path)
            logger.info("\n" + "-" * 70)
            logger.info("v7 (55 features) vs v8 (65 features + NTC) — Quantile Comparison")
            logger.info("-" * 70)
            logger.info(f"  {'Metric':<25} {'v7':>10} {'v8':>10} {'Delta':>10}")
            logger.info(f"  {'Mean Quantile Loss':<25} {v7_metrics['mean_quantile_loss']:>10.4f} {metrics['mean_quantile_loss']:>10.4f} {metrics['mean_quantile_loss']-v7_metrics['mean_quantile_loss']:>+10.4f}")
            logger.info(f"  {'CRPS':<25} {v7_metrics['crps']:>10.4f} {metrics['crps']:>10.4f} {metrics['crps']-v7_metrics['crps']:>+10.4f}")
            logger.info(f"  {'Coverage (80%)':<25} {v7_metrics['coverage_80pct']*100:>9.1f}% {metrics['coverage_80pct']*100:>9.1f}% {(metrics['coverage_80pct']-v7_metrics['coverage_80pct'])*100:>+9.1f}%")
            logger.info(f"  {'Crossing Rate':<25} {v7_metrics['crossing_rate']*100:>9.1f}% {metrics['crossing_rate']*100:>9.1f}% {(metrics['crossing_rate']-v7_metrics['crossing_rate'])*100:>+9.1f}%")
            logger.info(f"  {'Binary F1':<25} {v7_metrics['binary_f1']:>10.3f} {metrics['binary_f1']:>10.3f} {metrics['binary_f1']-v7_metrics['binary_f1']:>+10.3f}")

        # v6 classifier comparison (binary metrics)
        v6_metrics_path = MODELS_DIR / "test_metrics_v6.pkl"
        if v6_metrics_path.exists():
            v6_metrics = joblib.load(v6_metrics_path)
            logger.info("\n" + "-" * 70)
            logger.info("v6 (XGBoost Classifier) vs v8 (Quantile + NTC) — Binary Comparison")
            logger.info("-" * 70)
            logger.info(f"  {'Metric':<20} {'v6':>10} {'v8':>10} {'Delta':>10}")
        ...
        logger.info(f"  Quantiles: {DEFAULT_QUANTILES}")
        logger.info(f"  Features: {len(feature_cols)} (incl. {len(ntc_features)} NTC)")
        logger.info(f"  Mean Quantile Loss: {metrics['mean_quantile_loss']:.4f}")
        logger.info(f"  CRPS: {metrics['crps']:.4f}")
