"""
Create Feature Matrix v5 for NegaPriceNL Model Training

Pipeline:
1. Load the unified v5 dataset (15-min resolution)
2. Apply v5 feature engineering (D-1 12:00 safe)
...
def load_unified_dataset() -> pd.DataFrame:
    """Load the v5 unified dataset."""
    input_path = PROCESSED_DATA_DIR / "unified_dataset_v5.csv"

    if not input_path.exists():
        raise FileNotFoundError(
            f"Unified v5 dataset not found at {input_path}. "
            "Run data_preparation_v5.py first."
        )

    logger.info(f"Loading unified v5 dataset from {input_path}")
...
def handle_missing_values(df: pd.DataFrame, feature_cols: list) -> pd.DataFrame:
    ...
    - Forward-fill weather forecast gaps (max 6 QH = 1.5h)
...
def validate_features(df: pd.DataFrame, engine: NegativePriceFeatureEngineV5):
    ...
    if 'forecast_res_penetration' in df.columns:
        rp = df['forecast_res_penetration']
        logger.info(f"  forecast_res_penetration: [{rp.min():.3f}, {rp.max():.3f}]")
...
def main():
    logger.info("=" * 70)
    logger.info("NEGAPRICENL FEATURE MATRIX v5 CREATION")
    logger.info("=" * 70)
    ...
    try:
        ...
        engine = NegativePriceFeatureEngineV5()

        # Apply feature engineering
        logger.info("\nApplying v5 feature engineering...")
        ...
        # Save
        output_path = PROCESSED_DATA_DIR / "feature_matrix_v5.csv"
        ...
        # Summary
        logger.info("\n" + "=" * 70)
        logger.info("FEATURE MATRIX v5 SUMMARY")
        logger.info("=" * 70)
        ...
        logger.info("\n" + "=" * 70)
        logger.info("FEATURE MATRIX v5 CREATION COMPLETE")
        logger.info("=" * 70)
