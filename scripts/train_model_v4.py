"""
Train Gradient Boosting Model v4 for Negative Price Prediction

This version uses LEAK-FREE features (62 engineered features) with:
- All rolling statistics shifted by 1 to exclude current observation
- Momentum features redefined to use only lagged values
- Negative hour counts shifted to exclude current target
- No features use current price or current target value
"""
...
def train_final_model(
    ...
    logger.info("\nTraining final model v4 with best parameters...")
...
def evaluate_on_test(model: NegativePriceXGBoost, X_test, y_test) -> dict:
    ...
    logger.info("\n" + "=" * 70)
    logger.info("TEST SET EVALUATION (v4 - Enhanced Features)")
    logger.info("=" * 70)
...
def save_artifacts(model: NegativePriceXGBoost, metrics: dict, feature_cols: list, X_val, y_val):
    ...
    logger.info("\nSaving model v4 artifacts...")

    # Ensure output directory exists
    MODELS_DIR.mkdir(parents=True, exist_ok=True)

    # Save model with v4 suffix
    model_path = MODELS_DIR / "gradient_boost_negative_price_v4.pkl"
    model.save(model_path)
    logger.info(f"  Model saved to: {model_path}")

    # Save threshold separately for easy access
    threshold_path = MODELS_DIR / "optimal_threshold_v4.pkl"
    ...
    importance_path = MODELS_DIR / "feature_importance_v4.csv"
    importance.to_csv(importance_path, index=False)
    logger.info(f"  Feature importance saved to: {importance_path}")

    # Save metrics
    metrics_path = MODELS_DIR / "test_metrics_v4.pkl"
    joblib.dump(metrics, metrics_path)
    logger.info(f"  Metrics saved to: {metrics_path}")

    # Save feature list
    features_path = MODELS_DIR / "feature_columns_v4.pkl"
...
def main():
    """Main training function for v4 model."""
    logger.info("=" * 70)
    logger.info("NEGAPRICENL MODEL TRAINING v4 (ENHANCED FEATURES)")
    logger.info("=" * 70)
    logger.info(f"Start: {datetime.now()}")
    logger.info("Using: HistGradientBoostingClassifier (scikit-learn)")
    logger.info("Features: 62 LEAK-FREE engineered features (rolling stats & counts shifted)")
    ...
    try:
        ...
        logger.info("\n" + "=" * 70)
        logger.info("TRAINING v4 COMPLETE")
        logger.info("=" * 70)
