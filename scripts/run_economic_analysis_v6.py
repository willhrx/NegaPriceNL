"""
Run Economic Analysis for Negative Price Predictions (v6 - XGBoost)

This script:
1. Loads the v5 test data (2025, 15-min resolution)
2. Compares XGBoost ML model v6 against benchmark strategies
3. Calculates economic value for a 10 MW solar park
4. Generates comparison report and visualizations
5. Includes v5 vs v6 comparison

Usage:
    python scripts/run_economic_analysis_v6.py
"""
...
# === Benchmark strategies (same as v5) ===
...
def create_visualizations(results: pd.DataFrame, backtester: EconomicBacktester, test_df: pd.DataFrame):
    ...
    plt.tight_layout()
    fig.savefig(FIGURES_DIR / 'strategy_comparison_v6.png', dpi=150, bbox_inches='tight')
    plt.close()
    logger.info(f"  Saved: {FIGURES_DIR / 'strategy_comparison_v6.png'}")
    ...
    plt.tight_layout()
    fig.savefig(FIGURES_DIR / 'economic_waterfall_v6.png', dpi=150, bbox_inches='tight')
    plt.close()
    logger.info(f"  Saved: {FIGURES_DIR / 'economic_waterfall_v6.png'}")
    ...
    plt.tight_layout()
    fig.savefig(FIGURES_DIR / 'confusion_matrix_economic_v6.png', dpi=150, bbox_inches='tight')
    plt.close()
    logger.info(f"  Saved: {FIGURES_DIR / 'confusion_matrix_economic_v6.png'}")
    ...
    ax.plot(daily.index, daily['cum_savings_ml'], color='blue', linewidth=2,
            label=f'ML Model {MODEL_VERSION} (XGBoost)')
    ax.axhline(y=0, color='gray', linestyle='-', alpha=0.5, label='No Curtailment')

    ax.set_xlabel('Date', fontsize=12)
    ax.set_ylabel('Cumulative Savings (EUR)', fontsize=12)
    ax.set_title(f'Cumulative Savings Over 2025 Test Period\n({ASSET_CAPACITY_MW} MW Solar Park, v6 XGBoost, 15-min Resolution)', fontsize=14)
    ax.legend(loc='upper left')
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:,.0f}'))

    plt.tight_layout()
    fig.savefig(FIGURES_DIR / 'cumulative_savings_v6.png', dpi=150, bbox_inches='tight')
    plt.close()
    logger.info(f"  Saved: {FIGURES_DIR / 'cumulative_savings_v6.png'}")


def main():
    """Main economic analysis function."""
    logger.info("=" * 70)
    logger.info("NEGAPRICENL ECONOMIC ANALYSIS v6 (XGBoost)")
    logger.info("=" * 70)
    ...
    try:
        ...
        v5_report_path = REPORTS_DIR / "economic_analysis_v5.csv"

        comparison_lines = []
        comparison_lines.append("\n" + "=" * 70)
        comparison_lines.append("v5 (HistGradientBoosting) vs v6 (XGBoost) COMPARISON")
        comparison_lines.append("=" * 70)

        if v5_metrics_path.exists():
            v5_metrics = joblib.load(v5_metrics_path)
            v6_metrics_path = MODELS_DIR / f"test_metrics_{MODEL_VERSION}.pkl"
            v6_metrics = joblib.load(v6_metrics_path) if v6_metrics_path.exists() else None

            comparison_lines.append(f"  {'':20} {'v5':>12} {'v6':>12} {'Delta':>12}")
            comparison_lines.append(f"  {'Algorithm':<20} {'HistGBoosting':>12} {'XGBoost':>12} {'':>12}")
            comparison_lines.append(f"  {'Features':<20} {'55':>12} {'55':>12} {'0':>12}")

            if v6_metrics:
                comparison_lines.append(f"  {'Recall':<20} {v5_metrics['recall']*100:>11.1f}% {v6_metrics['recall']*100:>11.1f}% {(v6_metrics['recall']-v5_metrics['recall'])*100:>+11.1f}%")
                comparison_lines.append(f"  {'Precision':<20} {v5_metrics['precision']*100:>11.1f}% {v6_metrics['precision']*100:>11.1f}% {(v6_metrics['precision']-v5_metrics['precision'])*100:>+11.1f}%")
                comparison_lines.append(f"  {'F1':<20} {v5_metrics['f1']:>12.3f} {v6_metrics['f1']:>12.3f} {v6_metrics['f1']-v5_metrics['f1']:>+12.3f}")

        if v5_report_path.exists():
            v5_econ = pd.read_csv(v5_report_path)
            v5_ml = v5_econ[v5_econ['strategy'].str.contains('ML')]
            v6_ml = results[results['strategy'].str.contains('ML')]

            if len(v5_ml) > 0 and len(v6_ml) > 0:
                v5_cr = v5_ml.iloc[0]['capture_rate_pct']
                v6_cr = v6_ml.iloc[0]['capture_rate_pct']
                comparison_lines.append(f"  {'Capture Rate':<20} {v5_cr:>11.1f}% {v6_cr:>11.1f}% {v6_cr-v5_cr:>+11.1f}%")
        else:
            comparison_lines.append("  (v5 economic analysis not available for comparison)")
        ...
        logger.info("\n" + "=" * 70)
        logger.info("ECONOMIC ANALYSIS v6 COMPLETE")
        logger.info("=" * 70)

        ml_row = results[results['strategy'].str.contains('ML')].iloc[0]
        baseline_row = results[results['strategy'] == 'Never Curtail'].iloc[0]

        logger.info(f"\nKey Results for {MODEL_VERSION} Model (XGBoost):")
