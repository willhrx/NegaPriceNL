"""
Run Quantile Forecast Analysis and Economic Comparison (v8)

Evaluates the v8 quantile model (trained on v6 feature matrix with NTC features).

This script:
1. Loads the trained quantile regressor (v8)
2. Generates quantile price forecasts for the 2025 test period
3. Derives P(price<0), P(price<-10), interval widths
4. Bridges to binary decisions for economic comparison with v6/v7
5. Creates quantile-specific visualizations
6. Compares v8 vs v7 quantile metrics

Usage:
    python scripts/run_quantile_analysis_v8.py
"""
...
def main():
    ...
    try:
        ...
        if v7_metrics_path.exists():
            ...
            logger.info(f"\n  v7 vs v8 Quantile Comparison:")
            logger.info(f"    CRPS:      v7={v7_metrics['crps']:.4f}  v8={metrics['crps']:.4f}  delta={metrics['crps']-v7_metrics['crps']:+.4f}")
            logger.info(f"    Mean QL:   v7={v7_metrics['mean_quantile_loss']:.4f}  v8={metrics['mean_quantile_loss']:.4f}  delta={metrics['mean_quantile_loss']-v7_metrics['mean_quantile_loss']:+.4f}")
            logger.info(f"    Coverage:  v7={v7_metrics['coverage_80pct']*100:.1f}%  v8={metrics['coverage_80pct']*100:.1f}%")
            logger.info(f"    Crossing:  v7={v7_metrics['crossing_rate']*100:.1f}%  v8={metrics['crossing_rate']*100:.1f}%")
        ...
        strategies = [
            ...
            QuantileMLStrategy(model, feature_cols, version="v8"),
        ]

        # Add v7 quantile model if available
        v7_model_path = MODELS_DIR / "quantile_regressor_v7.pkl"
        v7_features_path = MODELS_DIR / "feature_columns_v7.pkl"
        if v7_model_path.exists() and v7_features_path.exists():
            v7_model = NegativePriceQuantileRegressor.load(v7_model_path)
            v7_feature_cols = joblib.load(v7_features_path)
            # Only add if all v7 features exist in test_df
            if all(c in test_df.columns for c in v7_feature_cols):
                strategies.append(QuantileMLStrategy(v7_model, v7_feature_cols, version="v7"))
        ...
        # Summary
        q_ml = results[results['strategy'].str.contains('v8')]
        if len(q_ml) > 0:
            row = q_ml.iloc[0]
            logger.info(f"\nQuantile ML v8 (median<0) Economic Results:")
