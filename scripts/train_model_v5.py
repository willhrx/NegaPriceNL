"""
Train Gradient Boosting Model v5 for Negative Price Prediction

This version uses only D-1 12:00 CET safe features at 15-min resolution:
- 55 forecast-based + lagged features (no delivery-day actuals)
- 15-minute Market Time Unit resolution (~175k train samples)
- Designed for operational deployment at day-ahead auction time
"""
...
# Features to exclude from training (target, metadata, delivery-day actuals)
EXCLUDE_COLUMNS = [
    'price_eur_mwh',         # Target source (leakage)
    'is_negative_price',     # Target variable
    'price_is_15min',        # Metadata flag
    'solar_generation_mw',   # Delivery-day actual (backtesting only)
    'wind_generation_mw',    # Delivery-day actual (backtesting only)
    'load_mw',               # Delivery-day actual (backtesting only)
    'flow_NL_DE_mw',         # Raw flow (D-2 mean already in snapshot features)
]


def load_feature_matrix() -> pd.DataFrame:
    """Load the v5 feature matrix."""
...
def train_final_model(
    ...
    logger.info("\nTraining final model v5 with best parameters...")
...
def evaluate_on_test(model: NegativePriceXGBoost, X_test, y_test) -> dict:
    ...
    logger.info("\n" + "=" * 70)
    logger.info("TEST SET EVALUATION (v5 - D-1 12:00 Safe Features, 15-min)")
    logger.info("=" * 70)
...
def save_artifacts(model: NegativePriceXGBoost, metrics: dict, feature_cols: list, X_val, y_val):
    ...
    logger.info("\nSaving model v5 artifacts...")

    # Ensure output directory exists
    MODELS_DIR.mkdir(parents=True, exist_ok=True)

    # Save model with v5 suffix
    model_path = MODELS_DIR / "gradient_boost_negative_price_v5.pkl"
    model.save(model_path)
    logger.info(f"  Model saved to: {model_path}")

    # Save threshold separately for easy access
    threshold_path = MODELS_DIR / "optimal_threshold_v5.pkl"
    ...
    importance_path = MODELS_DIR / "feature_importance_v5.csv"
    importance.to_csv(importance_path, index=False)
    logger.info(f"  Feature importance saved to: {importance_path}")

    # Save metrics
    metrics_path = MODELS_DIR / "test_metrics_v5.pkl"
    joblib.dump(metrics, metrics_path)
    logger.info(f"  Metrics saved to: {metrics_path}")

    # Save feature list
    features_path = MODELS_DIR / "feature_columns_v5.pkl"
...
def main():
    """Main training function for v5 model."""
    logger.info("=" * 70)
    logger.info("NEGAPRICENL MODEL TRAINING v5 (D-1 12:00 SAFE, 15-MIN)")
    logger.info("=" * 70)
    logger.info(f"Start: {datetime.now()}")
    logger.info("Using: HistGradientBoostingClassifier (scikit-learn)")
    logger.info("Features: 55 D-1 12:00 CET safe features (forecast-based + lagged)")
    ...
    try:
        ...
        if v4_metrics_path.exists():
            ...
            logger.info("-" * 70)
            logger.info(f"  {'Metric':<20} {'v4':>10} {'v5':>10} {'Delta':>10}")
