"""
Train XGBoost Quantile Regression Model v9

Same architecture as v7/v8 (5 quantile XGBRegressors) but trained on the
v7 feature matrix which includes price-anchoring features to address
systematic upward bias in v8.

New features in v7 (Phase 1):
- price_d1_same_qh: D-1 same quarter-hour price (24h lag)
- is_negative_d1_same_qh: D-1 negative indicator
- price_same_hour_7d_mean: Rolling 7-day same-hour mean

Features: 68 D-1 12:00 CET safe features (v6: 65 + Phase 1: 3)
Target: price_eur_mwh (continuous)

Usage:
    python scripts/train_quantile_model_v9.py
"""
...
EXCLUDE_COLUMNS = [
    ...
    'load_mw',
    'flow_NL_DE_mw',
    # Raw NTC per-direction columns (not features — use derived features instead)
...
def evaluate_on_test(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"TEST SET EVALUATION ({MODEL_VERSION} - Quantile Regression + NTC)")
    logger.info("=" * 70)
...
def main():
    ...
    logger.info("Features: 68 D-1 12:00 CET safe features (v7 feature matrix with price-anchoring)")
    ...
    try:
        ...
        # Leakage check
        actual_cols = [
            'solar_generation_mw', 'wind_generation_mw', 'load_mw',
        ...
        # Log new v7 price-anchoring features being used
        price_anchor_features = ['price_d1_same_qh', 'is_negative_d1_same_qh', 'price_same_hour_7d_mean']
        present_anchors = [c for c in price_anchor_features if c in feature_cols]
        logger.info(f"Price-anchoring features ({len(present_anchors)}): {present_anchors}")
        ...
        # Price-anchoring feature importance (v7 Phase 1 features)
        price_anchor_features = ['price_d1_same_qh', 'is_negative_d1_same_qh', 'price_same_hour_7d_mean']
        price_imp = importance[importance['feature'].isin(price_anchor_features)]
        if len(price_imp) > 0:
            logger.info(f"\nPrice-Anchoring Feature Importance (Phase 1):")
            for _, row in price_imp.iterrows():
                logger.info(f"  {row['feature']}: {row['importance']:.4f}")

        # v8 comparison (quantile metrics) - key comparison for bias reduction
        v8_metrics_path = MODELS_DIR / "test_metrics_v8.pkl"
        if v8_metrics_path.exists():
            v8_metrics = joblib.load(v8_metrics_path)
            logger.info("\n" + "=" * 70)
            logger.info("v8 (65 features + NTC) vs v9 (68 features + Price Anchors) — Quantile Comparison")
            logger.info("=" * 70)
            logger.info(f"  {'Metric':<25} {'v8':>10} {'v9':>10} {'Delta':>10}")
            logger.info(f"  {'Mean Quantile Loss':<25} {v8_metrics['mean_quantile_loss']:>10.4f} {metrics['mean_quantile_loss']:>10.4f} {metrics['mean_quantile_loss']-v8_metrics['mean_quantile_loss']:>+10.4f}")
            logger.info(f"  {'CRPS':<25} {v8_metrics['crps']:>10.4f} {metrics['crps']:>10.4f} {metrics['crps']-v8_metrics['crps']:>+10.4f}")
            logger.info(f"  {'Coverage (80%)':<25} {v8_metrics['coverage_80pct']*100:>9.1f}% {metrics['coverage_80pct']*100:>9.1f}% {(metrics['coverage_80pct']-v8_metrics['coverage_80pct'])*100:>+9.1f}%")
            logger.info(f"  {'Crossing Rate':<25} {v8_metrics['crossing_rate']*100:>9.1f}% {metrics['crossing_rate']*100:>9.1f}% {(metrics['crossing_rate']-v8_metrics['crossing_rate'])*100:>+9.1f}%")
            logger.info(f"  {'Binary F1':<25} {v8_metrics['binary_f1']:>10.3f} {metrics['binary_f1']:>10.3f} {metrics['binary_f1']-v8_metrics['binary_f1']:>+10.3f}")

            # Calibration improvement (most critical metric for bias reduction)
            if 'calibration' in v8_metrics and 'calibration' in metrics:
                ...
                logger.info(f"\n  CALIBRATION BIAS REDUCTION (PRIMARY METRIC):")
                logger.info(f"  {'q50 Observed Below':<25} {v8_q50*100:>9.1f}% {v9_q50*100:>9.1f}% {(v9_q50-v8_q50)*100:>+9.1f}%")
                logger.info(f"  {'Absolute Deviation':<25} {v8_bias:>9.1f}% {v9_bias:>9.1f}% {v9_bias-v8_bias:>+9.1f}%")
                if v9_bias < v8_bias:
        ...
        logger.info(f"  Quantiles: {DEFAULT_QUANTILES}")
        logger.info(f"  Features: {len(feature_cols)} (v6: 65 + Phase 1 price anchors: 3)")
        logger.info(f"  Mean Quantile Loss: {metrics['mean_quantile_loss']:.4f}")
        logger.info(f"  CRPS: {metrics['crps']:.4f}")
