"""
Train Gradient Boosting Model v3 for Negative Price Prediction

This version uses enhanced features (62 engineered features) including:
- Short-term price lags (1h, 2h, 4h, 6h, 12h)
- Momentum features (price change, generation ramping)
- Enhanced rolling statistics
- Time-of-day buckets
- Improved interaction terms
"""
...
def train_final_model(
    ...
    logger.info("\nTraining final model v3 with best parameters...")
...
def evaluate_on_test(model: NegativePriceXGBoost, X_test, y_test) -> dict:
    ...
    logger.info("\n" + "=" * 70)
    logger.info("TEST SET EVALUATION (v3 - Enhanced Features)")
    logger.info("=" * 70)
...
def save_artifacts(model: NegativePriceXGBoost, metrics: dict, feature_cols: list, X_val, y_val):
    ...
    logger.info("\nSaving model v3 artifacts...")

    # Ensure output directory exists
    MODELS_DIR.mkdir(parents=True, exist_ok=True)

    # Save model with v3 suffix
    model_path = MODELS_DIR / "gradient_boost_negative_price_v3.pkl"
    model.save(model_path)
    logger.info(f"  Model saved to: {model_path}")

    # Save threshold separately for easy access
    threshold_path = MODELS_DIR / "optimal_threshold_v3.pkl"
    ...
    # Save metrics
    metrics_path = MODELS_DIR / "test_metrics_v3.pkl"
    joblib.dump(metrics, metrics_path)
    logger.info(f"  Metrics saved to: {metrics_path}")

    # Save feature list
    features_path = MODELS_DIR / "feature_columns_v3.pkl"
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("NEGAPRICENL MODEL TRAINING v3 (ENHANCED FEATURES)")
    logger.info("=" * 70)
    logger.info(f"Start: {datetime.now()}")
    logger.info("Using: HistGradientBoostingClassifier (scikit-learn)")
    logger.info("Features: 62 engineered features including momentum & time buckets")
    ...
    try:
        ...
        logger.info("\n" + "=" * 70)
        logger.info("TRAINING v3 COMPLETE")
        logger.info("=" * 70)
